...
$desiredMappings =  @(
    ...
    @{"displayName"="Sharepoint Site A";"targetLocationType"="driveletter";"targetLocationPath"="Z:";"sourceLocationPath"="https://lieben.sharepoint.com/sites/lieben/Gedeelde%20%20documenten/Forms/AllItems.aspx";"mapOnlyForSpecificGroup"=""}
    @{"displayName"="Sharepoint Site B";"targetLocationType"="converged";"targetLocationPath"="Y:";"sourceLocationPath"="https://lieben.sharepoint.com/sites/lieben/Gedeelde%20%20documenten/Forms/AllItems.aspx";"mapOnlyForSpecificGroup"="AD Group SPSB"} 
    @{"displayName"="Sharepoint Site C";"targetLocationType"="converged";"targetLocationPath"="Y:";"sourceLocationPath"="https://lieben.sharepoint.com/sites/lieben/Gedeelde%20%20documenten/Forms/AllItems.aspx";"mapOnlyForSpecificGroup"="AD Group SPSC"} 
)
#>
...
$autoRerun             = $True                     #automatically rerun if a connection is dropped / lost but an active internet connect exists
...
while($true){
    $intendedMappings = $desiredMappings
    ...
    if($intendedMappings | Where-Object{$_.mapOnlyForSpecificGroup.Length -gt 0}){
        try{
            ...
            $intendedMappings = $intendedMappings | Where-Object{$_.mapOnlyForSpecificGroup.Length -eq 0}
        }
    }
    ...
    for($count=0;$count -lt $intendedMappings.Count;$count++){
        ...
        if($intendedMappings[$count].sourceLocationPath -ne "autodetect"){
            if($WebAssemblyloaded){
                $intendedMappings[$count].webDavPath = [System.Web.HttpUtility]::UrlDecode($intendedMappings[$count].sourceLocationPath)
            }
            $intendedMappings[$count].webDavPath = $intendedMappings[$count].webDavPath.Replace("https://","\\").Replace("/_layouts/15/start.aspx#","").Replace("sharepoint.com/","sharepoint.com@SSL\DavWWWRoot\").Replace("/Forms/AllItems.aspx","")
            $intendedMappings[$count].webDavPath = $intendedMappings[$count].webDavPath.Replace("/","\")  
        }else{
            $intendedMappings[$count].webDavPath = $mapURLpersonal
        }

        if($intendedMappings[$count].mapOnlyForSpecificGroup -and $groups){
            $group = $groups -contains $intendedMappings[$count].mapOnlyForSpecificGroup
            if($group){ 
                log -text "adding a sharepoint mapping because the user is a member of $($intendedMappings[$count].mapOnlyForSpecificGroup)" 
                $intendedMappings[$count].alreadyMapped = $False 
            }else{
                $intendedMappings[$count].alreadyMapped = $True
            }
        }
    }
    ...
    for($count=0;$count -lt $intendedMappings.Count;$count++){
        if($intendedMappings[$count].alreadyMapped){continue}
    ...
    if($autoRerun){
        log "autoRerun is set to True, OnedriveMapper will continue to monitor your mappings and remap if they get disconnected"
        :escape while($true){
            foreach($mapping in $intendedMappings | where {$_.mapped}){
                $mappingConnected = $True
                if(!(Test-Path $mapping.webDavPath)){
                    $mappingConnected = $False
                    Write-Host "UNHEALTHY: $($mapping.webDavPath)" -ForegroundColor Red
                }else{
                    Write-Host "HEALTHY: $($mapping.webDavPath) " -ForegroundColor Green
                }
                if(!$mappingConnected){
                    log "autoRerun is set to True and $($mapping.displayName) seems to be unavailable...checking internet connectivity"
                    $waitedForInternet = 0
                    ...
                    $secs = (Get-Random -Minimum 30 -Maximum 60)
                    Write-Host "Sleeping for $secs seconds" -ForegroundColor Green
                    Start-Sleep -s $secs
                } 
            }
        }
        log "autoRerun triggered, closing and reconnecting all mappings"
    }else{
        break
    }
    $firstRun = $False
