...
<#
.SYNOPSIS
    OneDriveMapper v6.10 - Maps OneDrive for Business and SharePoint libraries as network drives.
...
.DESCRIPTION
    ...
    All scalar settings (and the mapping tables as JSON) can be overridden centrally without
    editing this script by deploying registry values (e.g. via Intune or GPO) under:
    ...
    Value name = configuration variable name (e.g. 'O365CustomerName'). Booleans as DWORD 0/1,
    numbers as DWORD, text as REG_SZ. 'desiredMappings' and 'listOfFoldersToRedirect' as a
    REG_SZ containing a JSON array.
...
.PARAMETER Diagnose
    Runs environment diagnostics only (Edge, PRT, WebClient service, tenant connectivity,
    existing mappings) and exits without making any changes. Ideal for helpdesk triage.
...
DRIVE MAPPING CONFIGURATION
  ...
  targetLocationType      = 'driveletter' (maps to X:, Y:, etc)
                            'networklocation' (creates a shortcut in %USERPROFILE%\Links, usually visible in Quick Access)
                            'converged' (attempts both methods)
  targetLocationPath      = For 'driveletter': Drive letter (e.g. 'X:')
                            For 'networklocation': Base folder for shortcuts (ignored in Windows 10/11+, uses Quick Access)
                            For 'converged': Drive letter (primary method)
...
$edgeWaitSeconds       = 10                   # Max seconds to wait for silent PRT SSO (polls, exits early on success)
...
$reauthIntervalHours   = 12                   # Proactively re-authenticate every N hours to refresh cookies before they expire (0 = disabled; requires $autoRemapMethod enabled)

# OPTIONAL - Logging
$logfile               = Join-Path $env:APPDATA 'OneDriveMapper.log'
$maxLocalLogSizeMB     = 2
...
function Add-SiteToIEZone {
    ...
    try {
        ...
        if ($parts.Count -gt 2) {
            ...
            $null = New-ItemProperty "$base\$domain\$($parts[0])" -Name 'https' -Value $Mode -Force -ErrorAction Stop
        }
        else {
            $domain = "$($parts[0]).$($parts[1])"
            $null = New-Item "$base\$domain" -ErrorAction SilentlyContinue
            $null = New-ItemProperty "$base\$domain" -Name 'https' -Value $Mode -Force -ErrorAction Stop
        }
        return $true
    }
    catch { return -1 }
...
function Test-WebClient {
    ...
    if (-not $svc) {
        Write-Log -Text 'CRITICAL: WebClient service is not installed (WebDAV redirector missing - on Server OS install the WebDAV Redirector feature)' -IsError
        $script:errorsForUser += "Drive mapping failed: WebDAV (WebClient service) is not installed`n"
        return
    }
...
function Test-InternetConnectivity {
    <#
    .SYNOPSIS
        Tests TCP 443 reachability to the tenant's SharePoint host (proxy/firewall friendly,
        unlike ICMP pings to public DNS servers which many enterprises block).
    #>
    [CmdletBinding()]
    param(
        [string]$TargetHost = "$O365CustomerName.sharepoint.com",
        [int]$TimeoutMs = 5000
    )
    $client = $null
    try {
        $client = New-Object System.Net.Sockets.TcpClient
        $iar = $client.BeginConnect($TargetHost, 443, $null, $null)
...
function Import-RegistryConfiguration {
    <#
    .SYNOPSIS
        Applies configuration overrides from the registry so enterprises can manage this
        script centrally (Intune/GPO) without editing the file itself.
    .DESCRIPTION
        Reads values from Software\Policies\Lieben\OneDriveMapper in HKCU, then HKLM
        (machine policy wins). Value names must match configuration variable names.
        Booleans are DWORD 0/1, numbers DWORD, text REG_SZ. The mapping tables
        (desiredMappings / listOfFoldersToRedirect) are REG_SZ values containing a JSON array.
    .OUTPUTS
        Array of strings describing each applied override (logged by the caller once logging is up).
    #>
    [CmdletBinding()]
    param()

    $applied = [System.Collections.Generic.List[string]]::new()
    $jsonSettings = @('desiredMappings', 'listOfFoldersToRedirect')
    $overridableSettings = @(
        'O365CustomerName', 'redirectFolders', 'showConsoleOutput', 'autoRemapMethod', 'restartExplorer',
        'libraryName', 'displayErrors', 'persistentMapping', 'urlOpenAfter', 'removeExistingMaps',
        'removeEmptyMaps', 'autoDetectProxy', 'addShellLink', 'createUserFolderOn', 'convergedDriveLabel',
        'o365UserLogin', 'showProgressBar', 'progressBarColor', 'progressBarText', 'showSystemTrayIcon',
        'edgeWaitSeconds', 'fallbackToVisibleAuth', 'visibleAuthTimeout', 'reauthIntervalHours',
        'maxLocalLogSizeMB', 'onedriveIconPath', 'sharepointIconPath', 'logfile'
    )

    foreach ($hive in @('HKCU', 'HKLM')) {
        $polPath = "${hive}:\Software\Policies\Lieben\OneDriveMapper"
        if (-not (Test-Path $polPath)) { continue }
        $props = Get-ItemProperty -Path $polPath -ErrorAction SilentlyContinue
        if (-not $props) { continue }
        foreach ($prop in $props.PSObject.Properties) {
            try {
                if ($jsonSettings -contains $prop.Name) {
                    $parsed = @($prop.Value | ConvertFrom-Json | ForEach-Object {
                        $h = @{}
                        foreach ($jsonProp in $_.PSObject.Properties) { $h[$jsonProp.Name] = $jsonProp.Value }
                        $h
                    })
                    if ($parsed.Count -gt 0) {
                        Set-Variable -Name $prop.Name -Value $parsed -Scope Script
                        $applied.Add("$($prop.Name) ($($parsed.Count) entries) <- $hive policy")
                    }
                }
                elseif ($overridableSettings -contains $prop.Name) {
                    $current  = Get-Variable -Name $prop.Name -Scope Script -ValueOnly
                    $newValue = if ($current -is [bool]) { [bool][int]$prop.Value }
                                elseif ($current -is [int]) { [int]$prop.Value }
                                else { [string]$prop.Value }
                    Set-Variable -Name $prop.Name -Value $newValue -Scope Script
                    $applied.Add("$($prop.Name) = '$newValue' <- $hive policy")
                }
            }
            catch {
                $applied.Add("FAILED to apply $hive policy value '$($prop.Name)': $_")
            }
        }
    }
    return $applied
...
function Get-CdpCookiesForHost {
    <#
    .SYNOPSIS
        Connects to a CDP page WebSocket and returns all browser cookies for the given host.
    .OUTPUTS
        Array of cookie objects, or $null on failure / no cookies yet.
    #>
    [CmdletBinding()]
    param(
    ...
    try {
        ...
        $ws.ConnectAsync([Uri]$WebSocketDebuggerUrl, $cts.Token).Wait()
...
function Get-SharePointCookiesViaCDP {
    ...
    try {
        ...
        # Poll for authentication: done as soon as FedAuth/rtFa cookies exist in the browser.
        # Exits early on fast SSO (typically 3-6s) instead of always sleeping the full wait,
        # and never grabs a page before its cookies have actually been set (no races).
        $maxWait = if ($Headless) { $WaitSeconds + 15 } else { $ManualLoginTimeout }
        if ($Headless) { Write-Log -Text "Waiting up to ${maxWait}s for silent SSO (early exit on success)..." }
        else           { Write-Log -Text "Waiting for user to sign in (timeout: ${maxWait}s)..." }
        ...
        while ((Get-Date) -lt $deadline) {
            try {
                ...
                $spPage = $pages | Where-Object { $_.url -like "https://$spHost/*" -and $_.webSocketDebuggerUrl } | Select-Object -First 1
                if ($spPage) {
                    $lastCookies = Get-CdpCookiesForHost -WebSocketDebuggerUrl $spPage.webSocketDebuggerUrl -SpHost $spHost
                    if ($lastCookies) {
                        $fedAuth = ($lastCookies | Where-Object { $_.name -eq 'FedAuth' }).value
                        $rtFa    = ($lastCookies | Where-Object { $_.name -eq 'rtFa' }).value
                        if ($fedAuth -and $rtFa) { break }
                        $fedAuth = $null
                        $rtFa    = $null
                    }
                }
        ...
        if (-not $fedAuth -or -not $rtFa) {
            if ($Headless) {
                Write-Log -Text "Silent SSO did not complete for $spHost within ${maxWait}s" -IsWarning
            }
            else {
                Write-Log -Text "Authentication timed out after ${maxWait}s" -IsError
            }
            if ($lastCookies) {
                Write-Log -Text "FedAuth/rtFa not found among $(@($lastCookies).Count) cookies:" -IsWarning
                foreach ($c in $lastCookies) {
        ...
        try {
            ...
            $updatedPage  = $updatedPages | Where-Object { $_.url -like "https://$spHost/*" -and $_.webSocketDebuggerUrl } | Select-Object -First 1
...
function Set-WinINETCookies {
    ...
    try {
        # FedAuth: host-specific. 'secure' restricts the cookie to https so it can never
        # leak over plain http - the WebDAV path uses @SSL (https) so this is safe.
        $fedAuthUrl  = "https://$SpHost"
        $fedAuthData = "$FedAuth;Expires=$expiry;secure"
        ...
        $rtFaData = "$RtFa;Expires=$expiry;secure"
...
function Add-NetworkLocation {
    ...
    # Modern Windows 10/11 approach: Add to Quick Access instead of deprecated Network Shortcuts
    # The old CLSID={0AFACED1-E828-11D1-9187-B532F1E9575D} method no longer works on modern Windows
    try {
        # Validate inputs
        if ([string]::IsNullOrWhiteSpace($NetworkLocationName)) {
            Write-Error "NetworkLocationName cannot be empty"
            return $false
        }

        if ([string]::IsNullOrWhiteSpace($NetworkLocationTarget)) {
            Write-Error "NetworkLocationTarget cannot be empty"
            return $false
        }

        # Create shortcut in user's Links folder (feeds into Quick Access)
        $linksFolder = "$env:APPDATA\Microsoft\Windows\Recent\Links"
        if (-not (Test-Path $linksFolder -PathType Container)) {
            $linksFolder = "$env:USERPROFILE\Links"
        }
        
        if (-not (Test-Path $linksFolder -PathType Container)) {
            Write-Error "Could not locate Links folder for creating Quick Access shortcut"
            return $false
        }

        $lnkFile = Join-Path $linksFolder "$NetworkLocationName.lnk"
        
        # Remove old shortcut if it exists
        if (Test-Path $lnkFile) { Remove-Item $lnkFile -Force -ErrorAction SilentlyContinue }
        ...
        $lnk.Description = "OneDrive/SharePoint mapping created by OneDriveMapper v$version"
        $lnk.Save()
        [void][Runtime.InteropServices.Marshal]::ReleaseComObject($shell)

        # Add to Quick Access via registry for immediate visibility
        $quickAccessPath = 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\FolderShortcuts'
        $regName = $NetworkLocationName -replace '[^\w\s-]', ''
        if ($regName.Length -eq 0) { $regName = 'NetworkLocation' }
        
        $null = New-Item -Path $quickAccessPath -Force -ErrorAction SilentlyContinue
        $null = New-ItemProperty -Path $quickAccessPath -Name $regName -Value $NetworkLocationTarget -PropertyType String -Force -ErrorAction SilentlyContinue

        Write-Log -Text "Network location added to Quick Access: '$NetworkLocationName' -> $NetworkLocationTarget"
        return $true
    }
    catch {
...
function Invoke-DriveMapping {
    ...
    switch ($DriveMapping.targetLocationType) {
        'driveletter' { return Invoke-DriveLetterMapping -DriveMapping $DriveMapping -PersistenceFlag $persist }
        'converged'   { return Invoke-ConvergedMapping -DriveMapping $DriveMapping -PersistenceFlag $persist }
        default       { return Invoke-NetworkLocationMapping -DriveMapping $DriveMapping -PersistenceFlag $persist }
    }
}

function Invoke-ConvergedMapping {
    <#
    .SYNOPSIS
        Maps a WebDAV path as a shortcut inside a converged (subst'ed) drive letter.
    .DESCRIPTION
        Converged drives bundle multiple SharePoint libraries under one drive letter: the
        letter is subst'ed to a local folder earlier in the script, and each mapping becomes
        a shortcut inside it. This function primes the WebDAV connection and creates the
        shortcut in the converged drive's root.
    #>
    [CmdletBinding()]
    param([hashtable]$DriveMapping, [string]$PersistenceFlag)

    try {
        Write-Log -Text "Converged mapping: '$($DriveMapping.displayName)' into $($DriveMapping.targetLocationPath)"

        # Prime the WebDAV connection
        $out = $null
        ...
        $lnkFolder = "$($DriveMapping.targetLocationPath)\"
        if (-not (Test-Path $lnkFolder)) {
            Write-Log -Text "Converged drive $($DriveMapping.targetLocationPath) is not available (subst failed?)" -IsError
            return $false
        }

        $icon    = if ($DriveMapping.sourceLocationPath -eq 'autodetect') { $onedriveIconPath } else { $sharepointIconPath }
        $lnkFile = Join-Path $lnkFolder "$($DriveMapping.displayName).lnk"
        if (Test-Path $lnkFile) { Remove-Item $lnkFile -Force -ErrorAction SilentlyContinue }

        $shell = New-Object -ComObject WScript.Shell
        $lnk   = $shell.CreateShortcut($lnkFile)
        $lnk.TargetPath = $DriveMapping.webDavPath
        if ($icon -and [IO.File]::Exists($icon)) { $lnk.IconLocation = "$icon, 0" }
        $lnk.Description = "OneDrive/SharePoint mapping created by OneDriveMapper v$version"
        $lnk.Save()
        [void][Runtime.InteropServices.Marshal]::ReleaseComObject($shell)

        if ((Test-Path $lnkFile) -and (Test-Path $DriveMapping.webDavPath)) {
            Write-Log -Text "Converged link created: $lnkFile"
            return $true
        }
        Write-Log -Text "Converged mapping verification failed for '$($DriveMapping.displayName)'" -IsError
        return $false
    }
    catch {
        Write-Log -Text "Converged mapping failed: $_" -IsError
        return $false
    }
}
...
function Invoke-ElevationBypass {
    ...
    $scriptFile = Join-Path $scriptPath 'OneDriveMapper.ps1'
    # Inner quotes must reach schtasks as literal \" so paths with spaces survive
    $taskCmd    = "Powershell.exe -NoProfile -ExecutionPolicy Bypass$windowArg -File \`"$scriptFile\`" -AsTask"
    $result     = schtasks /Create /SC ONCE /TN OneDriveMapper /IT /RL LIMITED /F /TR $taskCmd /ST 00:00 2>&1 | Out-String
...
# Apply central configuration overrides (Intune/GPO) before anything else uses the config
$configOverrides = @(Import-RegistryConfiguration)
$O365CustomerName = $O365CustomerName.ToLower() -replace '\.onmicrosoft\.com', ''
...
foreach ($override in $configOverrides) {
    Write-Log -Text "Config override: $override"
}

# Diagnostic mode: report environment health and exit without changing anything
if ($Diagnose) {
    Write-Log -Text '===== DIAGNOSTIC MODE: reporting only, no changes will be made ====='

    $diagEdge = Find-EdgePath
    if ($diagEdge) { Write-Log -Text "Edge found: $diagEdge" }
    else           { Write-Log -Text 'Microsoft Edge NOT found - authentication is impossible' -IsError }

    $null = Test-DeviceState

    $diagSvc = Get-Service -Name WebClient -ErrorAction SilentlyContinue
    if ($diagSvc) { Write-Log -Text "WebClient service: $($diagSvc.Status) (StartType: $($diagSvc.StartType))" }
    else          { Write-Log -Text 'WebClient service NOT installed - WebDAV drive mapping is impossible' -IsError }

    foreach ($suffix in @('', $privateSuffix)) {
        $diagTarget = "$O365CustomerName$suffix.sharepoint.com"
        if (Test-InternetConnectivity -TargetHost $diagTarget) {
            Write-Log -Text "TCP 443 to ${diagTarget}: OK"
        }
        else {
            Write-Log -Text "TCP 443 to ${diagTarget}: FAILED (firewall/proxy/DNS?)" -IsError
        }
    }

    Get-PSDrive -PSProvider FileSystem | Where-Object { $_.DisplayRoot } | ForEach-Object {
        Write-Log -Text "Existing network mapping: $($_.Name): -> $($_.DisplayRoot)"
    }

    Write-Log -Text "Configured mappings: $(@($desiredMappings).Count)"
    foreach ($m in $desiredMappings) {
        Write-Log -Text "  '$($m.displayName)' [$($m.targetLocationType)] $($m.targetLocationPath) <- $($m.sourceLocationPath)"
    }
    Write-Log -Text "Log file: $logfile"
    Write-Log -Text '===== DIAGNOSTIC MODE complete ====='
    exit
}
    ...
    :escape while ($true) {
        foreach ($mapping in $intendedMappings) {
            ...
            $linkExists = switch ($mapping.targetLocationType) {
                'networklocation' { Test-Path $mapping.webDavPath -ErrorAction SilentlyContinue }
                'driveletter'     { Test-Path $mapping.targetLocationPath }
                'converged'       { Test-Path (Join-Path $mapping.targetLocationPath "$($mapping.displayName).lnk") }
                default           { $true }
            }
            ...
            if (-not $healthy) {
                ...
                if (-not (Test-InternetConnectivity)) {
                    Write-Log -Text "Tenant unreachable (TCP 443 to $O365CustomerName.sharepoint.com failed) - waiting 10s before retry" -IsWarning
                    Start-Sleep -Seconds 10
                    break
                }

                Write-Log -Text 'Tenant reachable - triggering remap'
                if ($script:traySync) {
                    $script:traySync.BalloonTitle = 'OneDriveMapper'
        ...
        # Proactive cookie refresh: FedAuth/rtFa cookies expire after some hours; re-authenticate
        # silently before that happens so drives never go stale overnight or after sleep/resume
        if ($reauthIntervalHours -gt 0 -and ((Get-Date) - $script:lastAuthTime).TotalHours -ge $reauthIntervalHours) {
            Write-Log -Text "Proactive re-authentication: last authentication was over $reauthIntervalHours hour(s) ago, refreshing cookies"
            break escape
        }
    }
