...
function handleMFArequest{
    ...
    try{
        $res = New-WebRequest -url "https://login.microsoftonline.com/common/SAS/BeginAuth" -Method POST -customHeaders $customHeaders -body ($body | ConvertTo-Json) -referer $res.ResponseUri.AbsoluteUri -contentType "application/json"
    ...
    while($true){
        ...
        try{
            ...
            $res2 = New-WebRequest -url "https://login.microsoftonline.com/common/SAS/EndAuth" -Method POST -customHeaders $customHeaders -body ($body | ConvertTo-Json) -referer $res.ResponseUri.AbsoluteUri -contentType "application/json"
    ...
    try{
        ...
        $res = New-WebRequest -url "https://login.microsoftonline.com/common/SAS/ProcessAuth" -Method POST -body $body -referer $res.ResponseUri.AbsoluteUri
        return $res
    }catch{
...
function New-WebRequest{
    ...
    while($true){
        ...
        try{
            ...
            # Compatibility: .NET Core (PowerShell 7+) does not have ResponseUri on WebResponse, but HttpWebResponse does
            if ($response -is [System.Net.HttpWebResponse]) {
                $retVal.ResponseUri = $response.ResponseUri
            } elseif ($response.PSObject.Properties['ResponseUri']) {
                $retVal.ResponseUri = $response.ResponseUri
            } else {
                $retVal.ResponseUri = $null
            }
...
function handleDuoMFA{
    ...
    try{
        ...
        $res = New-WebRequest -url $res.ResponseUri.AbsoluteUri -Method POST -body $body -customHeaders $customHeaders -referer $res.ResponseUri.AbsoluteUri -accept "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
    }catch{
    ...
    try{
        ...
        if($id_token -and $id_token -ne -1){
            ...
            $res = New-WebRequest -url "https://login.microsoftonline.com/common/federation/OAuth2ClaimsProvider" -Method POST -body $body -customHeaders $customHeaders -referer $res.ResponseUri.AbsoluteUri -accept "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
            return $res
        }
    }catch{
    ...
    try{
        $uri = $res.ResponseUri.AbsoluteUri.Replace("?sid","/data?sid")
        $res = New-WebRequest -url $uri -Method GET -customHeaders $customHeaders -referer $res.ResponseUri.AbsoluteUri -accept "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
    }catch{
    ...
    #return
    try{
        $sid = $res.ResponseUri.AbsoluteUri.Split("?")[1]
    ...
    #get auth method
    try{
        $sid = $res.ResponseUri.AbsoluteUri.Split("=")[1].Split("&")[0]
        $initiatePostUri = "https://$($redirectToDuoUrl2.Split("/")[2])/frame/v4/auth/prompt/data?post_auth_action=OIDC_EXIT&sid=$sid"
        $res = New-WebRequest -url $initiatePostUri -Method GET -customHeaders $customHeaders -referer $res.ResponseUri.AbsoluteUri -accept "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
    ...
    if($mfaPhone){
        ...
        try{
            ...
            $res = New-WebRequest -url "https://$($api)/frame/v4/prompt" -Method POST -body $body -customHeaders $customHeaders -referer $res.ResponseUri.AbsoluteUri -accept "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
        ...
        while($true){
            ...
            try{
                ...
                $res = New-WebRequest -url "https://$($api)/frame/v4/status" -Method POST -body $body -customHeaders $customHeaders -referer $res.ResponseUri.AbsoluteUri -accept "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
        ...
        try{
            ...
            $res = New-WebRequest -url "https://$($api)/frame/v4/oidc/exit" -Method POST -body $body -customHeaders $customHeaders -referer $res.ResponseUri.AbsoluteUri -accept "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
        }catch{
...
function checkIfAtFhmPage{
    ...
    if($value -eq -1){
        ...
        $body = "t=$value"
        try{
            $res = New-WebRequest -url $nextURL -Method POST -body $body -referer $res.ResponseUri.AbsoluteUri -contentType "application/x-www-form-urlencoded" -accept "text/html, application/xhtml+xml, image/jxr, */*"       
            $value = returnEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"t`" id=`"t`" value=`""
            if($value -ne -1){
                return $res
            }else{
...
function handleO365Redirect{
    ...
    $ear_jwe = returnEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"ear_jwe`" value=`""
    ...
    if($ear_jwe -ne -1){
        $body = "ear_jwe=$([System.Web.HttpUtility]::UrlEncode($ear_jwe))&state=$state"
        log -text "Detected a ear_jwe redirect, following..."
        try{
            $res = New-WebRequest -url $nextURL -Method POST -body $body -referer $res.ResponseUri.AbsoluteUri -contentType "application/x-www-form-urlencoded" -accept "text/html, application/xhtml+xml, image/jxr, */*"  
...
function loginV2(){
    ...
    if($res.ResponseUri.AbsoluteUri -like "*/personal/*"){
    ...
    try{
        if($adfsSmartLink){
            ...
            try{
                $res = New-WebRequest -url "https://login.microsoftonline.com/common/GetCredentialType" -Method POST -body $JSON -customHeaders $customHeaders -referer $res.ResponseUri.AbsoluteUri
    ...
    if($mode -eq "New_Managed"){
        ...
        while($true){
            ...
            if($res.ResponseUri.AbsoluteUri.StartsWith("https://login.microsoftonline.com")){
                #still at login page
                if($res.Content.IndexOf("<meta name=`"PageID`" content=`"KmsiInterrupt`"") -ne -1){
                    ...
                    try{
                        ...
                        $res = New-WebRequest -url "https://login.microsoftonline.com/kmsi" -Method POST -body $body -referer $res.ResponseUri.AbsoluteUri        
                    }catch{$Null}
                }
            }

            if($res.ResponseUri.AbsoluteUri.StartsWith("https://device.login.microsoftonline.com")){
                log -text "Deviceauth encountered, breaking auth loop"
                break
            }

            $code = returnEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"code`" value=`""
            if($res.ResponseUri.AbsoluteUri.StartsWith("https://login.microsoftonline.com") -and $code -ne -1){
    ...
    if($mode -eq "Federated"){
        ...
        if(!$adfsSmartLink){
            try{
                $res = New-WebRequest -url $nextURL -Method GET -referer $res.ResponseUri.AbsoluteUri
            }catch{
        ...
        if($SAMLRequest -ne -1){
            ...
            try{
                $res = New-WebRequest -url $nextURL -Method POST -body $body -referer $res.ResponseUri.AbsoluteUri
            }catch{
            ...
            $nextURL = "https://$($res.ResponseUri.Host)$nextURL"
    ...
    if($res.ResponseUri.OriginalString.IndexOf("/personal/") -ne -1){
...
for($count=0;$count -lt $desiredMappings.Count;$count++){
    ...
    if($desiredMappings[$count].sourceLocationPath -eq "autodetect"){
        ...
        while($stillProvisioning){
            ...
            try{
                if($res.ResponseUri.OriginalString.IndexOf("/personal/") -ne -1){
                    $url = $res.ResponseUri.OriginalString
                    $stillProvisioning = $False
