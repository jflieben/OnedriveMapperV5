...
$loginformIntroText    = "Welcome to COMPANY NAME`nPlease enter your login and password" #used as introduction text when you set userLookupMode to 6
...
#detecteer niet met forms maar op url waar mogelijk, verschillende ADFS/Okta versies
    ...
    if($mode -eq "Managed"){
        ...
        if($azureADSSOEnabled){
            $nextURL2 = "https://autologon.microsoftazuread-sso.com/$($jsonRealmConfig.DomainName)/winauth/sso?desktopsso=true&client-request-id=$clientId"
        ...
        while($true){
            ...
            if($azureADSSOEnabled -and $ssoToken.Length -gt 10){
                $body = "login=$userUPN&passwd=&ctx=$stsRequest&flowToken=$flowToken&dssoToken=$ssoToken&canary=$canary"
            }else{
