        ...
        switch($userLookupMode){
            ...
            5 {
                if(([Environment]::UserName).Split("@").Count -eq 2){
                    ...
                    log -text "userlookupMode is set to 5 -> Using $userUPN from the currently locally logged in username" 
                }else{
                    $userUPN = (([Environment]::UserName)+"@"+$domain).ToLower()
                    log -text "userlookupMode is set to 5 -> Using $userUPN from the currently logged in username + $domain" 
                }    
            }
            default {
