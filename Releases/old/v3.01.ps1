...
function setCookies{
    ...
    foreach($cookie in $cookies){
        if(Get-Content -LiteralPath $cookie.FullName | Select-String -Pattern "$O365CustomerName-my.sharepoint.com"){
            Remove-Item $cookie -Force -ErrorAction SilentlyContinue
            continue
        }
        if(Get-Content -LiteralPath $cookie.FullName | Select-String -Pattern "sharepoint.com"){
            Remove-Item $cookie -Force -ErrorAction SilentlyContinue
        }
    }
    ...
    foreach($cookie in $cookies){
        foreach($find in $findMe){
            if(Get-Content -LiteralPath $cookie.FullName | Select-String -Pattern $find){
                $found++
            }
        }
    }
...
#detecteer niet met forms maar op url waar mogelijk, verschillende ADFS/Okta versies
    ...
    if($mode -eq "Federated"){
        ...
        $body = "wa=wsignin1.0&wsignin1.0=estsredirect=2&wresult=$wResult"
        $res = JosL-WebRequest -url $nextURL -Method POST -body $body        
    }
...
if($authMethod -eq "original" -and (checkRegistryKeyValue -basePath "HKLM:\Software\Policies\Microsoft\Internet Explorer\Main" -entryName "DisableFirstRunCustomize") -ne 1){
...
if($authMethod -eq "original" -and $autoKillIE){ 
...
if($authMethod -eq "original" -and $autoProtectedMode){ 
...
if($dontMapO4B -eq $False -and !$desiredMappings[0].alreadyMapped){
    if($authMethod -eq "original"){
        ...
        try{
            $res = JosL-WebRequest -url "https://$O365CustomerName-my.sharepoint.com" -method GET
