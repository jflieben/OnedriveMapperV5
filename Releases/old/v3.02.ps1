...
function JosL-WebRequest{
    ...
    while($true){
        ...
        try{
            ...
            if($cookies){
                $cookies = $cookies -ireplace '(?<=expires).+?(?=;)', ''
                ...
                if($cookies.Count -gt 0){
                    foreach($cookie in $cookies){
                        ...
                        $script:cookiejar.Add((Create-Cookie -name $name -value $value -domain $domain -path $path -HttpOnly $True -Secure $True))
                    }
                }
            }
...
function setCookies{
    ...
    <#$cookies = get-childitem $folder -include * -recurse -force | where {$_.Extension -eq ".cookie"}
    foreach($cookie in $cookies){
...
#detecteer niet met forms maar op url waar mogelijk, verschillende ADFS/Okta versies
    ...
    $uidEnc = [System.Web.HttpUtility]::HtmlEncode($userUPN)
    ...
    try{
        if($adfsSmartLink){
            ...
            if(!$existingRes){
                ...
                log -text "Attempting to log in again, as we were logged out for some reason?"
            }else{
                $res = $existingRes
            }
    ...
    if($mode -eq "Federated"){
        ...
        if($wResult -eq -1){
            log -text "Federation Services did not sign us in automatically, retrieving user credentials.." -fout
            $password = retrievePassword
            $passwordEnc = [System.Web.HttpUtility]::HtmlEncode($password)
        ...
        $body = "wa=wsignin1.0&wresult=$wResult&wctx=$wctx"
    ...
    $body = "t=$value"
    $res = JosL-WebRequest -url $nextURL -Method POST -body $body -referer $res.rawResponse.ResponseUri.AbsoluteUri -contentType "application/x-www-form-urlencoded" -accept "text/html, application/xhtml+xml, image/jxr, */*"       
    log -text "Logged in to Office 365!"
    return $True
}
