...
#Enterprise users:  Finally, enterprise users receive email notifications of new versions and they get support

#redirect Favorites //DONE?
#always unmap optionally
#real check if logged in when using authv
...
######## 
$version = "3.04"
$configurationID       = "00000000-0000-0000-0000-000000000000"#Don't modify this, only used for OnedriveMapper Cloud Edition
...
$userLookupMode        = 1                         #1 = Active Directory UPN, 2 = Active Directory Email, 3 = Azure AD Joined Windows 10, 4 = query user for his/her login, 5 = lookup by registry key 
$AzureAADConnectSSO    = $False                     #if set to True, will automatically remove AzureADSSO registry key before mapping, and then readd them after mapping. Otherwise, mapping fails because AzureADSSO creates a non-persistent cookie
...
function ResetLog{
    ...
    if ((Test-Path $logfile) -eq $True){
        #The log file exists
        try{
            $i_LocalLogSize=Get-Item $logfile
            $i_LocalLogSize=($i_LocalLogSize).Length
...
function returnQuoteEnclosedFormValue{
    Param(
        $res,
        $searchString,
        [Switch]$decode
    ...
    try{
        ...
        $end = "`""
        $searchLength = $res.Content.IndexOf($end,$startLoc)-$startLoc
...
function redirectMyDocuments{
    ...
    while($true){
        try{
            ...
            if(![System.IO.Directory]::Exists($myFavoritesNewPath)){
...
function labelDrive{ 
    ...
    if($lD_DriveLabel.Length -gt 0){ 
        ...
        try{ 
            $regURL = $lD_MapURL.TrimEnd("\").Replace("\","#")
...
function setCookies{
    ...
    if($retry){
        try{
            ...
            log -text "Unable to find out where your cookies are stored! This means we can't double-check if they were really created when we attempt to create them. $($Error[0])"
        }
    }
    $cookies = get-childitem $folder -include * -recurse -force | where {$_.Extension -eq ".cookie" -or $_.Extension -eq ".txt"}
    foreach($cookie in $cookies){
        if(Get-Content -Path $cookie.FullName | Select-String -Pattern "$O365CustomerName-my.sharepoint.com"){
    ...
    #check if it was properly created
        ...
        if($found -lt $findMe.Count){
            Throw "Found $found cookies out of the expected $($findMe.Count) cookies in the system"
        }
}
...
#detecteer niet met forms maar op url waar mogelijk, verschillende ADFS/Okta versies
    ...
    try{
        if($adfsSmartLink){
            ...
            $apiCanary = returnQuoteEnclosedFormValue -res $res -searchString "`"apiCanary`":`""
            $iwaEndpoint = returnQuoteEnclosedFormValue -res $res -searchString "iwaEndpointUrlFormat: `""
            $clientId = returnQuoteEnclosedFormValue -res $res -searchString "correlationId`":`""
            #vind session code en gebruik deze om het realm van de user te vinden
            $stsRequest = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"ctx`" value=`""
            $flowToken = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"flowToken`" value=`""
            $canary = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"canary`" value=`""
    ...
    if(!$adfsSmartLink){
        ...
        if($jsonRealmConfig.NameSpaceType -eq "Managed"){
            ...
            if($jsonRealmConfig.is_dsso_enabled){
                ...
                log -text "Additionally, Azure AD SSO PassThrough is enabled for your tenant"
            }
    ...
    if($mode -eq "Managed"){
        ...
        if($azureADSSOEnabled){
            $nextURL2 = "https://autologon.microsoftazuread-sso.com/liebensraum.nl/winauth/sso?desktopsso=true&client-request-id=$clientId"
            try{
                $res = JosL-WebRequest -url $nextURL2 -trySSO 0 -method GET -accept "text/html, application/xhtml+xml, image/jxr, */*" -referer "https://login.microsoftonline.com/" 
            }catch{$Null}
            ...
            if($JSON.apiCanary){
                log -text "Retrieving AADC SSO status token..."
            }else{
        ...
        while($true){
            ...
            if($attempts -gt 0){
                $loginFormPos = returnQuoteEnclosedFormValue -res $res -searchString "<form id=`"credentials`" method=`"post`" action=`"/common/login`">"
            ...
            if($azureADSSOEnabled){
                try{
                    ...
                    $flowToken = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"flowToken`" value=`""
                    $canary = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"canary`" value=`""
                    $apiCanary = returnQuoteEnclosedFormValue -res $res -searchString "`"apiCanary`":`"" #handle a potential -1 result here!
                    ...
                    $JSON = ConvertTo-Json20 -item $JSON
                    log -text "Checking AADC SSO as this is enabled for your tenant..."
    ...
    if($mode -eq "Federated"){
        ...
        $wResult = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"wresult`" value=`""
        if($wResult -eq -1){
            ...
            $nextURL = returnQuoteEnclosedFormValue -res $res -searchString "<form method=`"post`" id=`"loginForm`" autocomplete=`"off`" novalidate=`"novalidate`" onKeyPress=`"if (event && event.keyCode == 13) Login.submitLoginRequest();`" action=`"/" -decode
        ...
        $nextURL = returnQuoteEnclosedFormValue -res $res -searchString "<form method=`"POST`" name=`"hiddenform`" action=`""
        $wctx = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"wctx`" value=`""
        if($wctx -ne -1){
    ...
    $nextURL = returnQuoteEnclosedFormValue -res $res -searchString "form name=`"fmHF`" id=`"fmHF`" action=`"" -decode
    $nextURL = [System.Web.HttpUtility]::HtmlDecode($nextURL)
    $value = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"t`" id=`"t`" value=`""
    if($value -eq -1){
        log -text "We do not seem to have been properly redirected after signing in....attempting to continue anyway" -fout
        return $False
    }
    $body = "t=$value"
    try{
...
function autoUpdate{
    ...
    if($desiredVersion -gt $version){
        ...
        $req.Content = $req.Content -replace "00000000-0000-0000-0000-000000000000`"#Don", "$($configurationID)`"#Don"
        try{
        ...
        try{
            ...
            log -text "Failed to start new Onedrivemapper version automatically: $($Error[0])" -fout
            Exit
        }
    }
}
...
$objUser = New-Object System.Security.Principal.NTAccount($Env:USERNAME)
$strSID = ($objUser.Translate([System.Security.Principal.SecurityIdentifier])).Value
log -text "You are $strSID running on Windows $windowsVersion with IE $ieVersion"
...
if($configurationID -ne "00000000-0000-0000-0000-000000000000"){
    ...
    $redirectMyDocsName = $configuratorSettings.redirectMyDocumentsName
...
if($authMode -eq "native" -and $PSVersionTable.PSVersion.Major -le 2){
    log -text "ERROR: you're trying to use Native auth on Powershell V2 or lower, this will likely fail" -fout
}
...
"@
    try{
        ...
        $str = $dateTime.ToString("R")
        $res = [Cookies.setter]::SetWinINETCookieString("https://testdomainthatdoesnotexist.com","none","nothing;Expires=$str")
...
if($authMethod -ne "native"){
    ...
    $res = loginV2
    if($res -eq $False){abort_OM}
    if($True -ne $res){
        if($res.rawResponse.responseUri.OriginalString.StartsWith("https://login.microsoftonline.com/login.srf?wa=wsignin1.0&rpsnv") -or $res.rawResponse.responseUri.OriginalString.StartsWith("https://login.microsoftonline.com/login.srf?wa=wsignin1%2E0&rpsnv") -or $res.rawResponse.responseUri.OriginalString.StartsWith("https://login.microsoftonline.com:443/login.srf?wa=wsignin1.0&rpsnv")){
            log -text "We were redirected to the logon page after login, this could mean a PSv2 bug, attempting once more" -warning
            $res = loginV2 -tryAgainRes $res
        }    
    }
}
...
if($dontMapO4B -eq $False -and !$desiredMappings[0].alreadyMapped){
    if($authMethod -ne "native"){
        ...
        try{
            ...
            $nextURL = [System.Web.HttpUtility]::HtmlDecode((returnQuoteEnclosedFormValue -res $res -searchString "form name=`"fmHF`" id=`"fmHF`" action=`"" -decode))
            $value = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"t`" id=`"t`" value=`""
            $body = "t=$value"
        }catch{
        ...
        try{
            if((returnQuoteEnclosedFormValue -res $res -searchString "<form method=`"POST`" name=`"hiddenform`" action=`"" -decode) -ne -1){
                $nextURL = returnQuoteEnclosedFormValue -res $res -searchString "<form method=`"POST`" name=`"hiddenform`" action=`"" -decode
                $code = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"code`" value=`""
                $id_token = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"id_token`" value=`""
                $session_state = returnQuoteEnclosedFormValue -res $res -searchString "<input type=`"hidden`" name=`"session_state`" value=`""
            ...
            if($nextURL.Length -gt 10){
                ...
                $res = JosL-WebRequest -url $nextURL -Method POST -body $body
            }elseif($res.rawResponse.responseUri.OriginalString.StartsWith("https://login.microsoftonline.com/login.srf?wa=wsignin1.0&rpsnv") -or $res.rawResponse.responseUri.OriginalString.StartsWith("https://login.microsoftonline.com/login.srf?wa=wsignin1%2E0&rpsnv") -or $res.rawResponse.responseUri.OriginalString.StartsWith("https://login.microsoftonline.com:443/login.srf?wa=wsignin1.0&rpsnv")){
                log -text "We were redirected to the logon page after login, this could mean a PSv2 bug, attempting once more" -warning
                $res = loginV2 -tryAgainRes $res
            }else{
