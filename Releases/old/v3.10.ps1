...
function checkIfAtO365URL{
    ...
    try{
        ...
        log -text "Failed to detect/use logged in Tile in IE: $($Error[0])" -fout
    }
    return $False
}
...
function login(){ 
    ...
    if($userLookupMode -eq 3){
        try{
            checkAndUseLoggedInTileIE -userUPN $userUPN
        }catch{
            log -text "Failed to use logged in user Tile in IE" -fout
        }
    ...
    while($True){ 
        ...
        if($reditWaited -gt 1 -and (checkIfAtO365URL -userUPN $userUPN -finalURLs $finalURLs)){
            log -text "Detected an url that indicates we've been signed in automatically: $($script:ie.LocationURL)"
            $useADFS = $True
            break            
        }
