...
try{
    log -text "Loading Selenium driver ($($scriptPath)\WebDriver.dll)"
    $seleniumDriverPath = "$($scriptPath)\WebDriver.dll"
...
while($true){
    ...
    #load edge driver
    try{
        $edgeDriverPath = "$($scriptPath)\msedgedriver.exe"
        ...
        if($autoUpdateEdgeDriver){
            if($edgeVersion){
                if($curEdgeDriverVersion -and $edgeVersion.Split(".")[0] -eq $curEdgeDriverVersion.Split(".")[0]){
                    ...
                    log -text "Downloading latest version from $edgeDriverDownloadUrl to $scriptPath"
                    ...
                    Expand-Archive -Path $tempZipPath -DestinationPath $scriptPath -Force -ErrorAction Stop
                    Remove-Item $tempZipPath -Force
                    Remove-Item "$($scriptPath)\Driver_Notes" -Recurse -Force
                    log -text "Updated to version $latestEdgeDriverVersion from $edgeDriverDownloadUrl to $scriptPath"
                }
            }else{
            ...
            <#
            log -text "Checking latest Edge version with Microsoft..."
            $tempVersionFilePath = Join-Path $ENV:TEMP -ChildPath "edgeVersion.txt"
            Invoke-WebRequest -Uri "https://msedgewebdriverstorage.blob.core.windows.net/edgewebdriver/LATEST_STABLE" -Method GET -UseBasicParsing -OutFile $tempVersionFilePath
            $latestEdgeDriverVersion = Get-Content $tempVersionFilePath
            Remove-Item $tempVersionFilePath -Force
            if(!$latestEdgeDriverVersion){
                Throw "Invalid remote edge version information"
            }elseif(!$curEdgeDriverVersion -or $latestEdgeDriverVersion -gt $curEdgeDriverVersion){
                log -text "Got $latestEdgeDriverVersion of EdgeDriver, which is higher than local version, autoupdating..."
            }else{
                log -text "Got $latestEdgeDriverVersion of the EdgeDriver, we're already at the most recent version"  
                $autoUpdateEdgeDriver = $False 
            }
            #>
        }

        <#try{
            if($autoUpdateEdgeDriver -and $latestEdgeDriverVersion){         
                $edgeDriverDownloadUrl = "https://msedgewebdriverstorage.blob.core.windows.net/edgewebdriver/$latestEdgeDriverVersion/edgedriver_win64.zip"
                log -text "Downloading latest version from $edgeDriverDownloadUrl to $scriptPath"
                $tempZipPath = Join-Path $ENV:TEMP -ChildPath "edgedriver_win64.zip"
                Invoke-WebRequest -uri $edgeDriverDownloadUrl -OutFile $tempZipPath -Method Get -UseBasicParsing -ErrorAction Stop
                Expand-Archive -Path $tempZipPath -DestinationPath $scriptPath -Force
                Remove-Item $tempZipPath -Force
                Remove-Item "$($scriptPath)\Driver_Notes" -Recurse -Force
                log -text "Updated to version $latestEdgeDriverVersion from $edgeDriverDownloadUrl to $scriptPath"
            }
        }catch{
            #fail ODM if auto update fails where no old version already exists to fall back to
            if(!(Test-Path -Path $edgeDriverPath)){
                Throw $_
            }
        }#>
        ...
        $global:edgeOptions.AddAdditionalCapability("useAutomationExtension",$False)
        ...
        $global:edgeDriverService = [OpenQA.Selenium.Edge.EdgeDriverService]::CreateDefaultService($scriptPath,"msedgedriver.exe")
